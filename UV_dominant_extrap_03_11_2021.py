# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 20:35:55 2021
...
f = open('\\Users\\Owner\\Downloads\\UV_data_2.txt','r')
line_list =[]
for line in f:
  line = line.strip("\n")
...
def model_exp(x_values,params,b=0.15):
...
def parameters(x,y,b=0.15):
    """Finds initial parameters for model_exp 
    of the form y = ae^bK^2 + c
    Args:
    ...
    x1,x2,x3 = x
    y1,y2,y3 = y
    K1 = ((2*(x1+(3/2)))**(1/2))*(1/b)**2
    K2 = ((2*(x2+(3/2)))**(1/2))*(1/b)**2
    K3 = ((2*(x3+(3/2)))**(1/2))*(1/b)**2
    k1 = ((2*(x1+(3/2)))**(1/2))*(1/b)
    k2 = ((2*(x2+(3/2)))**(1/2))*(1/b)
    k3 = ((2*(x3+(3/2)))**(1/2))*(1/b)
    dK1 = k2-k1
    ...
    dq1 = dy1/dK1
    dq2 = dy2/dK2
    B = np.log(dq2/dq1)/(cK2-cK1)
    a1 = dy1/(np.exp(B*K2)-np.exp(B*K1))
    a2 = dy2/(np.exp(B*K3)-np.exp(B*K2))
    a = (a1+a2)/2
    c1 = y1-(a*np.exp(B*K1))
    c2 = y2-(a*np.exp(B*K2))
    c3 = y3-(a*np.exp(B*K3))
...
def exponential(x,b=0.15):
        K = ((2*(x+(3/2)))**(1/2))*(b**(-1))
        y = (curvefit1[1]*np.exp(-3.67432169e-03*(K**2))) + curvefit1[0]
        return y
...
plt.tight_layout()
plt.savefig('Ultraviolet_03_11_5.pdf')
plt.show()
